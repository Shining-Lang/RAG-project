...
CREATE INDEX IF NOT EXISTS idx_kb_department ON kb_knowledge_base(department_id) WHERE is_deleted = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_permission_subject ON kb_permission(subject_type, subject_id);
...
CREATE INDEX IF NOT EXISTS idx_doc_kb_id ON kb_document(kb_id) WHERE is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_doc_status ON kb_document(status) WHERE is_deleted = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_chunk_embedding ON kb_doc_chunk
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 128);

-- 全文检索索引
CREATE INDEX IF NOT EXISTS idx_chunk_content_tsv ON kb_doc_chunk USING GIN (content_tsv);
...
CREATE INDEX IF NOT EXISTS idx_session_user ON kb_chat_session(user_id, last_active_at DESC)
...
CREATE INDEX IF NOT EXISTS idx_message_session ON kb_chat_message(session_id, created_at);

...
public class EnhancedRetrieverService {
    ...
    private final com.lsn.ragkb.repository.DocChunkRepository chunkRepository;
    ...
    public List<HybridRetrieverService.ScoredChunk> retrieveWithHyde(
        ...
        List<HybridRetrieverService.ScoredChunk> originalResults =
                hybridRetriever.retrieveWithPermissionCheck(question, kbIds, vectorTopK);
        ...
        List<com.lsn.ragkb.entity.DocChunk> hydeResults = kbIds.stream()
        ...
        Map<Long, com.lsn.ragkb.entity.DocChunk> chunkMap = new HashMap<>();
        ...
        for (int rank = 0; rank < hydeResults.size(); rank++) {
            com.lsn.ragkb.entity.DocChunk chunk = hydeResults.get(rank);
            double rrfScore = 1.0 / (RRF_K + rank + 1);
